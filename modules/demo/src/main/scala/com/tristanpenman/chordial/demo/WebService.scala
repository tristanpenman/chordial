...
object WebService extends DefaultJsonProtocol {
  final case class NodeAttributes(nodeId: Long,
                                  successorId: Option[Long],
                                  active: Boolean)
...
trait WebService {
  ...
  private def getNodeAttributes(governor: ActorRef,
                                nodeId: Long): Future[NodeAttributes] =
    governor
      ...
      .map {
        ...
        case GetNodeStateInvalidRequest(message) =>
          throw new Exception(
            s"Governor rejected request for node state ($message)")
      }
      .flatMap {
        case active =>
          if (active) {
            governor
              ...
              .map {
                ...
                case GetNodeSuccessorIdInvalidRequest(message) =>
                  throw new Exception(
                    s"Governor rejected request for node successor ID ($message)")
              }
          } else {
            Future {
            ...
            case Some(seedId) =>
              ...
              onSuccess(future) {
                case CreateNodeWithSeedOk(nodeId, nodeRef) =>
                  complete(NodeAttributes(nodeId,
                                          Some(seedId.toLong),
                                          active = true).toJson.compactPrint)
