...
class ClosestPrecedingNodeAlgorithmSpec
  ...
  private def newAlgorithmActor(node: NodeInfo, pointersRef: ActorRef) =
    system.actorOf(
      ClosestPrecedingNodeAlgorithm.props(node, pointersRef, algorithmTimeout))
  ...
  "A ClosestPrecedingNodeAlgorithm actor" when {
      ...
      testCases.foreach {
        case input =>
          "return a pointer for Node with ID 1 when queried for ID " + input in {
            ...
            expectMsg(
              ClosestPrecedingNodeAlgorithmFinished(
    ...
    "initialised with a Node ID of 1 and a Pointers actor that has one finger table entry, with ID 3" should {
      ...
      def newPointersActor: ActorRef =
        TestActorRef(new Actor {
          override def receive: Receive = {
            case GetSuccessorList() =>
              sender() ! GetSuccessorListOk(NodeInfo(successorId,
                                                     dummyActorRef),
                                            List.empty)
          }
        })
      ...
      testCases.foreach {
        case (input, output) =>
          "return a pointer for Node with ID " + output + " when queried for ID " + input in {
            val algorithm = newAlgorithmActor(NodeInfo(nodeId, dummyActorRef),
                                              newPointersActor)
            algorithm ! ClosestPrecedingNodeAlgorithmStart(input)
            expectMsg(
              ClosestPrecedingNodeAlgorithmFinished(
                NodeInfo(output, dummyActorRef)))
          }
      }
    ...
    "initialised with a Pointers actor that is unresponsive" should {
      "return an error" in {
        newAlgorithmActor(NodeInfo(1L, dummyActorRef), unresponsiveActor) ! ClosestPrecedingNodeAlgorithmStart(
          1L)
