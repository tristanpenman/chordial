...
class NotifyAlgorithm extends Actor with ActorLogging {
  ...
  def awaitUpdatePredecessor(delegate: ActorRef): Receive = {
    ...
    case message =>
      log.warning(
        "Received unexpected message while waiting for UpdatePredecessorResponse: {}",
        message)
  }

  def awaitGetPredecessor(delegate: ActorRef,
                          node: NodeInfo,
                          candidate: NodeInfo,
                          pointersRef: ActorRef): Receive = {
    ...
    case message =>
      log.warning(
        "Received unexpected message while waiting for GetPredecessorResponse: {}",
        message)
  }
  ...
  override def receive: Receive = {
    case NotifyAlgorithmStart(node, candidate, pointersRef) =>
      ...
      context.become(
        awaitGetPredecessor(sender(), node, candidate, pointersRef))

    case message =>
      log.warning(
        "Received unexpected message while waiting for NotifyAlgorithmStart: {}",
        message)
  }
}
...
object NotifyAlgorithm {
  ...
  case class NotifyAlgorithmStart(node: NodeInfo,
                                  candidate: NodeInfo,
                                  pointersRef: ActorRef)

  sealed trait NotifyAlgorithmStartResponse

  case class NotifyAlgorithmAlreadyRunning()
      extends NotifyAlgorithmStartResponse

  case class NotifyAlgorithmOk(predecessorUpdated: Boolean)
      extends NotifyAlgorithmStartResponse

  case class NotifyAlgorithmError(message: String)
      extends NotifyAlgorithmStartResponse
